...
{
    class VillageSmartGridRestConsumer
    {
        static HttpClient _client = new HttpClient();
        static string _url = "http://localhost:53135/api/VillageSmartGrid";


        public static async Task<HttpResponseMessage> Put(int id, VillageSmartGridModel value)
        {
        ...
        public static async Task<HttpResponseMessage> Post(VillageSmartGridModel value)
        {
            var content = JsonConvert.SerializeObject(value);
            var buffer = Encoding.UTF8.GetBytes(content);
            var byteContent = new ByteArrayContent(buffer);
        ...
        public static async Task<List<VillageSmartGridModel>> GetAll()
        {
            var response = await _client.GetStringAsync(new Uri(_url));
            return JsonConvert.DeserializeObject<List<VillageSmartGridModel>>(response);
        }
